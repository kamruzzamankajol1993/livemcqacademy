...
@section('title') Chapter Management | {{ $ins_name ?? 'App' }} @endsection
                        ...
                        <div class="table-responsive">
                            <table class="table table-hover table-bordered align-middle">
                                <thead>
                                    <tr>
                                        <th>Sl</th>
                                        <th class="sortable" data-column="name_en">Chapter Name</th>
                                         <th>Class</th>
                                        <th>Subject</th>
                                        <th>Section</th>
                        ...
                        <ul id="sortable-list">
                            ...
                            <li data-id="{{ $item->id }}">
                                ...
                                <div class="d-flex gap-2">
                                    ...
                                    @if($item->section) <span class="badge bg-secondary">{{ $item->section->name_en }}</span> @endif
                                </div>
                            </li>
                            @endforeach
                        </ul>
...
<div class="modal fade" id="importModal" tabindex="-1">
    <div class="modal-dialog">
        <form action="{{ route('chapter.import') }}" method="POST" enctype="multipart/form-data" class="modal-content">
            @csrf
            <div class="modal-header"><h5 class="modal-title">Import Chapters</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
            <div class="modal-body">
                <div class="text-center mb-3">
                    <a href="{{ route('chapter.sample') }}" class="btn btn-info text-white btn-sm"><i class="fa fa-download me-1"></i> Download Sample</a>
                </div>
                <div class="mb-3">
                    <label>Upload Excel</label>
                    <input type="file" name="file" class="form-control" required accept=".xlsx,.xls,.csv">
                    <small class="text-muted">Columns: name_en, name_bn, subject_name, section_name</small>
                </div>
            </div>
...
<div class="modal fade" id="addModal" tabindex="-1">
    <div class="modal-dialog modal-lg">
        <form action="{{ route('chapter.store') }}" method="POST" class="modal-content">
            @csrf
            <div class="modal-header"><h5 class="modal-title">Add Chapter</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
            <div class="modal-body">
                <div class="row">
                    ...
                    {{-- 3. Section Selection (Dependent) --}}
                    <div class="col-md-4 mb-3">
                        <label>Section</label>
                        <select name="section_id" class="form-control select2-modal" style="width:100%">
        ...
        <form id="editForm" method="POST" class="modal-content">
            ...
            <div class="modal-body">
                <div class="row">
                    ...
                    <div class="col-md-4 mb-3">
                        <label>Section</label>
                        <select name="section_id" id="edit_section_id" class="form-control select2-modal" style="width:100%">
                            <option value="">Select Section (Optional)</option>
                        </select>
                    </div>
...
<script>
    $(document).ready(function() {
        ...
        var routes = {
            fetch: "{{ route('ajax.chapter.data') }}",
            store: "{{ route('chapter.store') }}",
            show: "{{ route('chapter.show', ':id') }}",
            update: "{{ route('chapter.update', ':id') }}",
            destroy: "{{ route('chapter.destroy', ':id') }}",
            reorder: "{{ route('chapter.reorder') }}",
            // Helper Routes
            getSubjects: "{{ route('ajax.get.subjects') }}",
            getSections: "{{ route('ajax.get.sections') }}",
            csrf: "{{ csrf_token() }}"
        };
        ...
        $('#addModal select[name="class_id"]').on('change', function() {
            ...
            var $sectionSelect = $('#addModal select[name="section_id"]');

            $subjectSelect.html('<option value="">Select Subject</option>').trigger('change');
            $sectionSelect.html('<option value="">Select Section (Optional)</option>').trigger('change');
        ...
        $('#addModal select[name="subject_id"]').on('change', function() {
            ...
            if (classId && subjectId) {
                $.get(routes.getSections, { class_id: classId, subject_id: subjectId }, function(data) {
                    var options = '<option value="">Select Section (Optional)</option>';
                    data.forEach(item => options += `<option value="${item.id}">${item.name_en}</option>`);
        ...
        $('#edit_class_id').on('change', function(e) {
            if (!e.originalEvent) return; // Programmatic change ignore
            var classId = $(this).val();
            var $subjectSelect = $('#edit_subject_id');
            var $sectionSelect = $('#edit_section_id');
            ...
            $.get(routes.fetch, { page: currentPage, search: searchTerm, sort: sortColumn, direction: sortDirection }, function(res) {
                ...
                if(res.data.length > 0) {
                    res.data.forEach((item, i) => {
                        ...
                        let clsName = item.class ? item.class.name_en : '--'; // Show Class
                        let subName = item.subject ? item.subject.name_en : '--';
                        let secName = item.section ? item.section.name_en : '--';
            ...
            $.get(showUrl, function(data) {
                ...
                if(data.class_id) {
                    $.get(routes.getSubjects, { class_id: data.class_id }, function(subjects) {
                        ...
                        // 4. Fetch Sections based on Class & Subject
                        if(data.subject_id) {
                            $.get(routes.getSections, { class_id: data.class_id, subject_id: data.subject_id }, function(sections) {
                                var secOptions = '<option value="">Select Section (Optional)</option>';
                                sections.forEach(s => secOptions += `<option value="${s.id}">${s.name_en}</option>`);
                                $('#edit_section_id').html(secOptions);
                                
                                // 5. Set Section
                                $('#edit_section_id').val(data.section_id).trigger('change.select2');
                            });
                        }
                    });
                }
        ...
        $('#addModal, #editModal').on('hidden.bs.modal', function() {
            ...
            if($(this).attr('id') === 'addModal') {
                ...
                $(this).find('select[name="section_id"]').html('<option value="">Select Section (Optional)</option>');
            }
        });
    });
</script>
