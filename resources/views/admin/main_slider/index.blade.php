...
<main class="main-content">
    <div class="container-fluid">
        <div class="d-flex justify-content-between align-items-center flex-wrap gap-3 mb-4">
            <h2 class="mb-0">Main Slider List</h2>
            <div class="d-flex align-items-center">
                 <form class="d-flex me-2" role="search">
                    <input class="form-control" id="searchInput" type="search" placeholder="Search by title..." aria-label="Search">
                </form>
                <a href="{{ route('main-slider.create') }}" class="btn text-white" style="background-color: var(--primary-color); white-space: nowrap;"><i data-feather="plus" class="me-1" style="width:18px; height:18px;"></i> Add New Slider</a>
            </div>
        </div>
        <div class="card">
            <div class="card-body">
...
$(document).ready(function() {
    ...
    var routes = {
        fetch: "{{ route('ajax.main-slider.data') }}",
        destroy: id => `{{ url('main-slider') }}/${id}`,
        csrf: "{{ csrf_token() }}"
    };
    ...
    function fetchData() {
        ...
        $.get(routes.fetch, { page: currentPage, search: searchTerm, sort: sortColumn, direction: sortDirection }, function (res) {
            ...
            if (res.data.length === 0) {
                rows = '<tr><td colspan="5" class="text-center">No sliders found.</td></tr>';
            } else {
                res.data.forEach((item, i) => {
                    const statusBadge = item.status == 1 ? '<span class="badge bg-success">Active</span>' : '<span class="badge bg-danger">Inactive</span>';
                    const editUrl = `{{ url('main-slider') }}/${item.id}/edit`;
                    const imageUrl = `{{ asset('/') }}public/${item.image}`;

                    rows += `<tr>
                        <td>${(res.current_page - 1) * 10 + i + 1}</td>
                        <td>${item.title}</td>
                        <td><img src="${imageUrl}" alt="${item.title}" width="150"></td>
                        <td>${statusBadge}</td>
                        <td>
            ...
            $('#tableBody').html(rows);
            // You can add pagination logic here
        });
    }
