...
@section('title', 'Package Payment Report')
...
<main class="main-content">
    <div class="container-fluid mt-3">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h4 class="fw-bold mb-0">Package Payment Reports</h4>
            <div class="dropdown">
                ...
                <ul class="dropdown-menu dropdown-menu-end shadow border-0">
                    ...
                    <li><a class="dropdown-item active-report" href="{{ route('reports.packagePayments') }}">Package Payments</a></li>
                </ul>
            </div>
        </div>
            ...
            <div class="card-body">
                ...
                <div class="row g-3 mb-4 p-3 bg-light rounded border">
                    ...
                    <div class="col-md-2 d-flex align-items-end">
                        <button class="btn btn-primary w-100 fw-bold" onclick="fetchData(1)">Filter</button>
                    </div>
                </div>
...
<script>
    ...
    function fetchData(page) {
        $.ajax({
            url: "{{ route('reports.packagePayments') }}",
    ...
    function renderTable(data, from) {
        ...
        if (data.length > 0) {
            data.forEach((item, i) => {
                let statusBadge = item.status === 'success' ? 'bg-success' : 'bg-danger';
                html += `<tr>
                    <td>${from + i}</td>
                    <td><strong>${item.user ? item.user.name : 'N/A'}</strong><br><small>${item.user ? item.user.phone : ''}</small></td>
                    ...
                    <td class="small">${item.transaction_id || item.trx_id}</td>
                    <td>${moment(item.created_at).format('DD MMM, YYYY')}</td>
                    <td class="text-center"><span class="badge ${statusBadge}">${item.status.toUpperCase()}</span></td>
                </tr>`;
            });
        } else {
            html = '<tr><td colspan="8" class="text-center py-4">No payment records found.</td></tr>';
        }
        $('#tableBody').html(html);
    }
