...
@section('title') Board Management | {{ $ins_name ?? 'App' }} @endsection
                        ...
                        <div class="table-responsive">
                            <table class="table table-hover table-bordered align-middle">
                                <thead>
                                    <tr>
                                        <th>Sl</th>
                                        <th class="sortable" data-column="name_en">Board Name (EN)</th>
                                        <th class="sortable" data-column="name_bn">Board Name (BN)</th>
...
<div class="modal fade" id="importModal" tabindex="-1">
    <div class="modal-dialog">
        <form action="{{ route('board.import') }}" method="POST" enctype="multipart/form-data" class="modal-content">
            @csrf
            <div class="modal-header"><h5 class="modal-title">Import Boards</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
            <div class="modal-body">
                <div class="text-center mb-3">
                    <a href="{{ route('board.sample') }}" class="btn btn-info text-white btn-sm"><i class="fa fa-download me-1"></i> Download Sample</a>
                </div>
                <div class="mb-3">
...
<div class="modal fade" id="addModal" tabindex="-1">
    <div class="modal-dialog">
        <form action="{{ route('board.store') }}" method="POST" class="modal-content">
            @csrf
            <div class="modal-header"><h5 class="modal-title">Add Board</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
            <div class="modal-body">
                <div class="mb-3">
...
<div class="modal fade" id="editModal" tabindex="-1">
    <div class="modal-dialog">
        <form id="editForm" method="POST" class="modal-content">
            @csrf @method('PUT')
            <div class="modal-header"><h5 class="modal-title">Edit Board</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
            <div class="modal-body">
                <div class="mb-3">
                    <label>Name (English) *</label>
...
<script>
    $(document).ready(function() {
        var routes = {
            fetch: "{{ route('ajax.board.data') }}",
            store: "{{ route('board.store') }}",
            show: "{{ route('board.show', ':id') }}",
            update: "{{ route('board.update', ':id') }}",
            destroy: "{{ route('board.destroy', ':id') }}",
            reorder: "{{ route('board.reorder') }}",
            csrf: "{{ csrf_token() }}"
        };
